...
class ChannelMuteDialog extends HookConsumerWidget {
  const ChannelMuteDialog({
    super.key,
        ...
        ElevatedButton(
          ...
          onPressed: () async {
            final result = await futureWithDialog(
              context,
              ref
                  .read(channelNotifierProvider(account, channelId).notifier)
                  .mute(
                    expiresAt: switch (period.value) {
                      _MutePeriod.indefinite => null,
                      _MutePeriod.at => expiresAt.value,
                      _MutePeriod.after => DateTime.now().add(
                        expiresAfter.value,
                      ),
                    },
                  )
